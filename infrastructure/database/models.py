...
class PlotLine(Base):
    ...
    id = Column(Integer, primary_key=True)
    name = Column(String(100), nullable=False)  # "Основной сюжет", "Любовь"
...
class Chapter(Base):
    ...
    content = Column(Text)  # текст главы
    context_summary = Column(Text)  # для передачи LLM
    generated_at = Column(DateTime, nullable=True)

    book = relationship("Book", back_populates="chapters")
    plot_events = relationship("PlotEvent", back_populates="chapter")

# Чтобы можно было использовать Session = sessionmaker()
engine = None
Session = None

def init_db(db_url: str = "sqlite:///storywriter.db"):
